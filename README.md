...
| 指标 | 数值 |
|---|---|
| Countdown 上 Qwen2.5-3B-Instruct 基线正确率（128 道测试题） | **4.69%** |
| GRPO 训练 140 步后峰值正确率 | **46.88%** |
| 相对提升 | **≈10×** |
| 绝对提升 | **+42.19 个百分点** |
| 训练步数 | 140（触发 early stopping，patience=5×10 步） |
| 训练 wall-clock 时间 / 单卡 GPU | ≈ 70 min / RTX PRO 6000 Blackwell 96GB |

> 评估方式：在 128 道留出测试题上，每题用当前策略采样 1 条回答，`answer_reward == 1.0`（表达式合法 + 数字各用一次 + 求值等于 target）的占比即正确率。

### 训练曲线

![评估正确率](docs/images/eval_accuracy.png)
*评估正确率（每 10 步一个点）：从基线 4.69% 一路爬到 46.88%，140 步内几乎线性增长。*

![平均奖励](docs/images/avg_reward.png)
*训练每步的平均奖励（含格式奖励 × 0.1 + 答案奖励）：曲线持续抬升，与评估正确率方向一致。*
