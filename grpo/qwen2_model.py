"""从零手写的 Qwen2 Transformer，专为 GRPO 训练 + 推理两用。

为什么不用 `transformers.Qwen2ForCausalLM`：
    1. GRPO 需要在 rollout 阶段长时间用 KV Cache 做自回归生成，再切回
       train 模式做梯度更新；HF 实现的 cache 接口偏向纯推理，复用麻烦。
    2. 自己写可以把 RoPE、GQA、RMSNorm、SafeTensors 加载、gradient
       checkpointing 等细节一次性看明白 —— 这正是简历"手写底层"那一条
       的核心产出。

模块对应：
    Qwen2Config         : 把 HF config.json 平铺为 dataclass，方便阅读
    RMSNorm             : 等价于 LayerNorm 但去掉了中心化 + 偏置
    Qwen2RotaryEmbedding: 预计算 cos/sin 频率表
    rotate_half / apply_rotary_pos_emb : RoPE 旋转的具体张量操作
    Attention           : GQA（K/V head 数 < Q head 数）+ KV Cache + SDPA
    FeedForward         : SwiGLU MLP，gate_proj * up_proj → SiLU → down_proj
    TransformerBlock    : Pre-Norm 残差结构（norm → attn → +res → norm → ffn → +res）
    Transformer         : 整体 forward（训练，带 checkpoint_sequential）与
                          inference（推理，带 start_pos & KV Cache）两套路径
    Transformer.from_pretrained : 直接从 HF 风格目录加载 safetensors 权重
"""
...
class RMSNorm(torch.nn.Module):
    """Root Mean Square LayerNorm（LLaMA/Qwen 系列标配）。

    与 LayerNorm 相比：
      - 不减均值（去掉了"中心化"，因为对 LM 这步收益小、开销不小）
      - 没有 bias 项（只有一个可学习的 scale `weight`）

    公式：y = weight * x / sqrt(mean(x^2) + eps)

    实现上把 x 提升到 fp32 计算 norm，再 cast 回原 dtype，避免 bf16 下
    平方和溢出/精度丢失。
    """
    ...
    def _norm(self, x):
        # rsqrt(x) = 1 / sqrt(x)，比先 sqrt 再 reciprocal 更快
        return x * torch.rsqrt(x.pow(2).mean(-1, keepdim=True) + self.eps)

    def forward(self, x):
        input_dtype = x.dtype
        x = x.to(torch.float32)  # fp32 算 norm，防 bf16 下平方和溢出
...
def rotate_half(x):
    """把最后一维"前半段"与"后半段"按 (-x2, x1) 拼接，用于 RoPE 的 90° 旋转分量。

    对二维向量 (a, b)，旋转矩阵作用：
        [cos -sin] [a]   [a cos - b sin]
        [sin  cos] [b] = [a sin + b cos]
    即 result = (a, b) * cos + (-b, a) * sin。
    这里 `rotate_half(x)` 正是把整段维度成对地从 (x1, x2) 翻成 (-x2, x1)。
    """
    x1 = x[..., : x.shape[-1] // 2]
    x2 = x[..., x.shape[-1] // 2 :]
...
def apply_rotary_pos_emb(q, k, cos, sin, unsqueeze_dim=2):
    """对 Q 和 K 同时施加 RoPE：x' = x * cos + rotate_half(x) * sin。

    cos/sin shape 是 (B, T, head_dim)；这里 `unsqueeze(2)` 在 num_heads 维
    上插一个 1，让广播到 (B, T, num_heads, head_dim) 的 q / k 上去。

    RoPE 的好处：相对位置只决定 attention 分数中两个位置的差，
    位置编码隐式注入到 QK 内积里，不占额外维度，也支持外推。
    """
...
class Attention(nn.Module):
    """Grouped-Query Attention + RoPE + KV Cache 的一体化实现。

    GQA：Q 有 n_heads 个头，K/V 只有 n_kv_heads 个头（n_kv_heads ≤ n_heads），
    每 n_rep = n_heads // n_kv_heads 个 Q 头共享一组 K/V。在 Qwen2.5-3B 里
    n_heads=16、n_kv_heads=2，K/V 显存只占 MHA 的 1/8，但效果几乎无损。
    PyTorch 2.5+ 的 `scaled_dot_product_attention(enable_gqa=True)` 已经原生
    支持 GQA，不必手动 repeat K/V。

    KV Cache：训练时 forward 不传 `start_pos`，走纯前向（带因果 mask）；
    推理时 `init_kv_cache()` 预分配 (B_max, T_max, n_kv_heads, head_dim)，
    每步把新 token 的 K/V 写入 `[start_pos : start_pos+seqlen]`，SDPA 只看
    `[:end_pos]` 即可重用历史 K/V。
    """

    def __init__(self, args: Qwen2Config):
        super().__init__()
        # 兼容 MHA：如果 config 没指定 kv heads，就退化到 n_heads
        self.n_kv_heads = (
        ...
        self.n_rep = self.n_heads // self.n_kv_heads  # 每组 K/V 被多少个 Q 头共享
