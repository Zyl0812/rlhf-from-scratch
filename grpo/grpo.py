"""GRPO 算法核心：rollout + 组内标准化 + 策略梯度更新。

GRPO（Group Relative Policy Optimization, DeepSeekMath 提出）与 PPO 的关键差异：

    PPO:  A_t = GAE_λ(r_t - V(s_t))              ← 需要训一个 critic V
    GRPO: A_i = (R_i - mean({R_j})) / std({R_j}) ← 同题 N 条回答的组内 Z-score

GRPO 把"同一问题采样多条回答"作为天然 baseline，省掉 critic，显存 ≈ ½ PPO，
特别适配 *规则奖励 + 多次采样* 场景（数学/代码/逻辑题）。

本文件三件事：

    rollout()                     : 并行采样 N 条回答，处理变长 prompt + 早停
    normalize_rewards_per_group() : 按 prefix 分组，做组内 Z-score 得到优势
    update_policy()               : 微批次累积梯度 + clip_grad_norm + step
"""
