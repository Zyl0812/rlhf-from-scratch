"""从零手写的 Qwen3-0.6B 参考实现，**仅作学习笔记**。

与 `grpo/qwen2_model.py` 相比，Qwen3 的关键变化：
    1. QK-Norm：对 Q 和 K 在 dot-product 之前各做一次 RMSNorm
       （`qk_norm=True`）。论文里说能稳定大模型训练。
    2. RoPE 的成对方式：本实现采用"前一半 ↔ 后一半"配对（即 (x0, x4),
       (x1, x5), ...），而非常见的相邻两两 (x0, x1), (x2, x3)。两种方式
       数学上等价，但与 HF 默认权重对齐时必须保持一致。
    3. RMSNorm `qwen3_compatible=True` 会强制 fp32 计算 norm，与官方
       一致；否则在 bf16 下会出现微小数值差。

实现要点：
    - `forward(x, mask, cos, sin, start_pos, cache)` 把整层所需要的全部
      位置/掩码/缓存都从外面传进来，方便上层做 KV Cache 管理。
    - `Qwen3Tokenizer` 是一个不依赖 transformers 的轻量 tokenizer：直接
      从 `tokenizer.json` 加载 BPE，并手写 chat template 包装函数。

运行：`python model_notes/qwen_3.py`（需先把 `Qwen3-0.6B/tokenizer.json` 放到工作目录）。
"""
...
def apply_rope(x, cos, sin, offset=0):
    ...
    sin = (
        sin[offset : offset + seq_len, :].unsqueeze(0).unsqueeze(0)
    )  # Shape: (1, 1, seq_len, head_dim)
